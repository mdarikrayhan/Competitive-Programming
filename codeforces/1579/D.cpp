...
void _solve(){
    int n; vi a; rvec(n, a);
    set<pii> ps; forn(i, n) if(a[i]) ps.emplace(a[i], i);
    vpii ans;
    while(sz(ps) > 1){
        auto [x, i] = *ps.rbegin(); ps.erase(prev(ps.end()));
        auto [y, j] = *ps.rbegin(); ps.erase(prev(ps.end()));
        ans.pb({i,j});
        if(x > 1) ps.emplace(x-1, i);
        if(y > 1) ps.emplace(y-1, j);
    }
    out(sz(ans));
    for(auto [i, j]: ans) out(i+1,j+1);
