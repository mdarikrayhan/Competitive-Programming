...
int n,m,s,t,dis[2005],vis[2005],diss[2005],dist[2005],p[2005],u,v,w;
...
void dij(int x)
{
	...
	memset(dis,127,sizeof(dis)); memset(vis,0,sizeof(vis)); dis[x]=0; Q.push(mp(0,x));
	while(!Q.empty())
	{
		int nr=Q.top().se; Q.pop(); if(vis[nr]) continue; vis[nr]=1;
		for(auto i:e[nr])
		{
			if(dis[nr]+i.se<dis[i.fi]) dis[i.fi]=dis[nr]+i.se,Q.push(mp(dis[i.fi],i.fi));
		}
	}
	//for(int i=1;i<=n;i++) cout<<dis[i]<<" "; cout<<endl;
}

int dp[2005][2005][2],mn[2005][2005][2];
int nxt[2005][2005][2];

int calcc(int xl,int xr,int yl,int yr)
{
	return cnt[xr][yr]-cnt[xl-1][yr]-cnt[xr][yl-1]+cnt[xl-1][yl-1];
}
int calcs(int xl,int xr,int yl,int yr)
{
...
signed main()
{
	...
	for(int i=1;i<=m;i++) cin>>u>>v>>w,e[u].pb(mp(v,w)),e[v].pb(mp(u,w));
	dij(s); for(int i=1;i<=n;i++) diss[i]=dis[i]; sort(dis+1,dis+n+1); for(int i=1;i<=n;i++) diss[i]=lower_bound(dis+1,dis+n+1,diss[i])-dis;
	dij(t); for(int i=1;i<=n;i++) dist[i]=dis[i]; sort(dis+1,dis+n+1); for(int i=1;i<=n;i++) dist[i]=lower_bound(dis+1,dis+n+1,dist[i])-dis;
	for(int i=1;i<=n;i++) cnt[diss[i]][dist[i]]++,sum[diss[i]][dist[i]]+=p[i];
	for(int i=1;i<=n;i++) for(int j=1;j<=n;j++) cnt[i][j]+=cnt[i-1][j]+cnt[i][j-1]-cnt[i-1][j-1];
	for(int i=1;i<=n;i++) for(int j=1;j<=n;j++) sum[i][j]+=sum[i-1][j]+sum[i][j-1]-sum[i-1][j-1];
	//for(int i=1;i<=n;i++) cout<<i<<" "<<diss[i]<<" "<<dist[i]<<endl;
	for(int j=0;j<=n;j++)
	{
		int lst=0;
		for(int i=n;i>=1;i--)
		{
			if(calcc(i,i,j+1,n)) lst=i;
			//cout<<j<<" "<<i<<" "<<calcc(i,i,j+1,n)<<endl;
			nxt[i-1][j][0]=lst; 
		}
	}
	for(int i=0;i<=n;i++)
	...
	for(int i=n;i>=0;i--)
	{
		for(int j=n;j>=0;j--)
		{
			if(!nxt[i][j][0]) dp[i][j][0]=0;
			else dp[i][j][0]=calcs(i+1,n,j+1,n)-mn[nxt[i][j][0]][j][1];
			if(!nxt[i][j][1]) dp[i][j][1]=0;
			else dp[i][j][1]=calcs(i+1,n,j+1,n)-mn[i][nxt[i][j][1]][0];
			mn[i][j][1]=min(mn[i+1][j][1],dp[i][j][1]);
			mn[i][j][0]=min(mn[i][j+1][0],dp[i][j][0]);
			//cout<<i<<" "<<j<<" "<<nxt[i][j][0]<<" "<<nxt[i][j][1]<<" "<<dp[i][j][0]<<" "<<dp[i][j][1]<<endl;
		}
	}
	int dt=dp[0][0][0]*2-calcs(1,n,1,n);
	if(dt>0) cout<<"Break a heart"<<endl;
	else if(dt==0) cout<<"Flowers"<<endl;
	else if(dt<0) cout<<"Cry"<<endl;
}////