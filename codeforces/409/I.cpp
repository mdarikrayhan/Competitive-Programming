...
void solve() {
  ...
  G.build();
  vc<int> V = toposort(G);
  if (V.empty()) return print("false");

  vc<int> ANS(len(X));
  for (auto& v: V) {
    for (auto& e: G[v]) { chmax(ANS[e.to], ANS[v] + 1); }
  }
  if (MAX(ANS) >= 10) return print("false");
  string out;
  for (auto& x: name) {
    int idx = LB(X, x);
    out += to_string(ANS[idx]);
  }
  print(out);
}
