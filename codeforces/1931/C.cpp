...
void solve(){ll n; cin >> n; vector<ll> a(n); for (ll i = 0; i < n; i++) { cin >> a[i]; } ll L = 0, R = n - 1; while (a[L] == a[0]) { ++L; } while (a[R] == a[n - 1]) { --R; } if (n == 1) { cout << 0 << '\n'; return; } if (a.front() != a.back()) { cout << min(n - L, R + 1) << nl; return; } cout << max(0LL, R - L + 1) << nl;}
