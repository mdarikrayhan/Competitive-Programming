...
ll djik(ve<ve<pil> >& adj,int src,int dest,int& n,int& m,vi& par,vll& dist){
    priority_queue<pli,ve<pli>,greater<pli> > pq;
    dist[src]=0ll;
    pq.push(mp(0ll,src));
    vb vis(n,false);
    while(!pq.empty()){
        ...
        f(i,sz){
            int newc=adj[curr][i].first;
            if(adj[curr][i].second+distt<dist[newc]){
                par[newc]=curr;
                dist[newc]=adj[curr][i].second+distt;
                pq.push(mp(dist[newc],newc));
            }
        }
    }
    if(!vis[dest]){
...
void dfs(vvi& adj,vi& arr,int curr,vll& least,int minn,vb& inst){
    // cout<<curr<<" ";
    if(least[curr]!=-1){
        return;
    }
    ...
    ll ans=0;
    f(i,n){
        if(arr[adj[curr][i]]<minn){
            dfs(adj,arr,adj[curr][i],least,minn,inst);
            ans=max(ans,least[adj[curr][i]]+1);
        } 
    }
    inst[curr]=false;
    least[curr]=ans;
...
int main()
{
    ...
    while(t--){
        ...
        while(high>low){
            ...
            f(i,n){
                if(arr[i]<mid){
                    dfs(adj,arr,i,least,mid,inst);
                    oper=max(oper,least[i]);
                }
