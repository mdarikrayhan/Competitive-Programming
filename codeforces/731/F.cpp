...
int count(int from, int to) {
    int total;
    if (to > 200000) total = n;

    else total = pre[to];

    int remove = pre[from - 1];
    return total - remove;
}
...
void solve() {
    ...
    vector<int> pussy(200001, 0);
    for (int x: arr) pussy[x] += 1;
    range(i, 1, 200001) pre[i] = pre[i - 1] + pussy[i];
...
//    print(pre, 50);
    ...
    for (int x: arr) {
        if (watch.count(x) == 1) continue;
        watch.insert(x);
        int curr = 0;
        for (int i = 1; i * x <= 200000; i++) {
            curr += (x * i * count(x * i, x * i + x - 1));

//            print(curr);
        }
        ans = max(ans, curr);
//        print(make_pair(curr, x));
    }
    print(ans);
