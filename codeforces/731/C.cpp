...
%:endif
const int N = 2000100;
int c[N], fa[N], vis[N], idx;
vector<int> scc[N], fat;
int find(int x) {
...
auto main() [[O3]] -> signed {
    ...
    F(i, 1, m) {
        int x, y; cin >> x >> y;
        if (!vis[x]) idx++, vis[x] = true;
        if (!vis[y]) idx++, vis[y] = true;
        if (find(x) != find(y)) fa[find(x)] = find(y);
    }
    F(i, 1, n) {
        scc[find(i)].pb(i);
        if (vis[i] && find(i) == i) fat.pb(i);
    }
    int res = 0;
    F(i, 0, (int)fat.size() - 1) {
        map<int, int> mp; int mx = 0;
        F(j, 0, scc[fat[i]].size() - 1) mx = max(mx, ++mp[c[scc[fat[i]][j]]]);
        res += mx;
    }
    cout << idx - res << '\n';
