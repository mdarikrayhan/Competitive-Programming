...
int main()
{
    fastio
        multicase
    {
        int number, minimum = INT_MAX, maximum = INT_MIN, answer = INT_MIN;
        char c;
        string s;
        cin >> number >> c >> s;
        ...
        for (int x = 0; x < number; x++)
        {
            if (s[x] == 'g')
            {
                greenvector.push_back(x);
            }
        }
        greenvector.push_back(greenvector[0] + number);
        for (int x = 0; x < number; x++)
        {
            if (s[x] == c)
            {
                int val = *lower_bound(greenvector.begin(), greenvector.end(), x);
                answer = max(answer, val - x);
            }
        }
        cout << answer << nl;
        sort(greenvector.begin(),greenvector.end());
        sort(greenvector.begin(),greenvector.end(),greater <int>());
        isPrime(number);
    }
    return 0;
}