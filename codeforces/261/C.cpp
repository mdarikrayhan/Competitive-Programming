...
void solve(int test){
    auto f=[&](int m){
        ...
        int cnt=0;
        for(int i=1; i<=m+1; i++)cout << a[m+1][i] << " ";
        for(int i=1; i<=m+1; i++)cnt+=a[m+1][i];
        cout << m+1 << " " << cnt << " " <<(1<<(__builtin_popcount(m+1)))<< endl;
    };
   ...
   int si=a.size();
   dp[0][0][0]=1;
   rep(i,si)rep(j,si+1)rep(is,2){
       ll now=dp[i][j][is];
       rep(next,2){
           if(!is && next>a[i])continue;
           int nis=is;
           if(next<a[i])nis=1;
           int nj=j+next;
           dp[i+1][nj][nis]+=now;
