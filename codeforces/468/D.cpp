...
int n,u,v,w,sz[100005],mx[100005],R,ans,p[100005];
...
void dfs0(int x,int fat)
{
	...
	for(auto i:e[x])
	{
		if(i.fi==fat) continue;
		dfs0(i.fi,x); ans+=min(sz[i.fi],n-sz[i.fi])*i.se*2; sz[x]+=sz[i.fi]; mx[x]=max(mx[x],sz[i.fi]);
	}
	mx[x]=max(mx[x],n-sz[x]); if(mx[x]<mx[R]) R=x;
}

int cntys,ys[100005],cval[100005];
set <int> num[100005];
set <pair<int,int>> dys;

void dfs1(int x,int fat)
{
	ys[x]=cntys; num[cntys].insert(x);
	for(auto i:e[x])
	{
		if(i.fi==fat) continue; dfs1(i.fi,x);
	}
}
...
#define ls(x) (x<<1)
#define rs(x) ((x<<1)|1)
#define n(x) tree[x].mn
#define md(x,y) ((x+y)>>1)
#define push_up(x) n(x)=min(n(ls(x)),n(rs(x)))

void build(int l,int r,int p)
{
	if(l==r) return n(p)=(*num[l].begin()),void(); int mid=md(l,r);
...
void cz(int l,int r,int x,int p)
{
	if(l==r)
	{
		if(!num[l].size()) n(p)=10000000; else n(p)=(*num[l].begin()); return;
	}
	int mid=md(l,r); if(mid>=x) cz(l,mid,x,ls(p)); else cz(mid+1,r,x,rs(p)); push_up(p);
}
int ask(int l,int r,int x,int y,int p)
{
	if(x>y) return 1000000000;
	if(l>=x&&r<=y) return n(p); int mid=md(l,r),na=1000000000;
	if(mid>=x) na=min(na,ask(l,mid,x,y,ls(p))); if(mid<y) na=min(na,ask(mid+1,r,x,y,rs(p))); return na;
}

int solve(int x,int lim)
{
	int to;
	if(cval[ys[x]]==lim||(*(--dys.end())).fi<lim||(*(--dys.end())).se==ys[R])
	{
		if(x==R) to=n(1);
		else to=min(ask(1,cntys,1,ys[x]-1,1),ask(1,cntys,ys[x]+1,cntys,1));
	}
	else
	{
		int mys=(*(--dys.end())).se;
	...
	num[ys[to]].erase(to); cz(1,cntys,ys[to],1);
	dys.erase(mp(cval[ys[x]],ys[x])); cval[ys[x]]--; dys.insert(mp(cval[ys[x]],ys[x]));
	dys.erase(mp(cval[ys[to]],ys[to])); cval[ys[to]]--; dys.insert(mp(cval[ys[to]],ys[to]));
	return to;
}
...
signed main()
{
	...
	mx[0]=10000000; dfs0(1,1); cout<<ans<<endl;
	for(auto i:e[R]) cntys++,dfs1(i.fi,R); cntys++; ys[R]=cntys; num[cntys].insert(R);
	for(int i=1;i<=n;i++) cval[ys[i]]+=2;
	for(int i=1;i<=cntys;i++) dys.insert(mp(cval[i],i));
	build(1,cntys,1);
	for(int i=1;i<=n;i++)
	{
		p[i]=solve(i,(n-i+1));
