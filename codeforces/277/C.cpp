...
void solve(int test){
    ...
    rep(i,k){
        ...
        if(Sx==Gx){
            if(Sy>Gy)swap(Sy,Gy);
            gx[Sx].push_back(P(Sy,Gy));
        }
        else {
            if(Sx>Gx)swap(Sx,Gx);
            gy[Sy].push_back(P(Sx,Gx));
        }
    }
    map<int,int> lenx;
    map<int,int> seenx;
    for(auto &&[fi,p]:gx){
        ...
        vp np;
        for(auto u:p){
            if(np.size()==0)np.push_back(u);
            else {
                if(np.back().second<u.first){
                    np.push_back(u);
                }
                else {
                    chmax(np.back().second,u.second);
                }
            }
        }
        p=np;
    ...
    int hx=n-1-lenx.size();
    int hy=m-1-leny.size();
    int xorv=0;
    if(hx%2)xorv^=m;
    if(hy%2)xorv^=n;
    for(auto u:lenx)xorv^=u.second;
    for(auto u:leny)xorv^=u.second;
    if(!xorv){
        cout << "SECOND" << endl;
        return;
    }
    ...
    if(hx && (xorv^m)<=m){
        int moku=m-(xorv^m);
        for(int i=1; i<=n-1; i++)if(!seenx[i]){
            cout <<i << " " <<  0 << " " << i << " " << moku << endl;
            return;
        }
    }
    ...
    if(hy && (xorv^n)<=n){
        int moku=n-(xorv^n);
        for(int i=1; i<=m-1; i++)if(!seeny[i]){
            cout << 0 << " " << i << " " << moku <<" " << i << endl;
            return;
        }
    }
    for(auto [fi,len]:lenx){
        if((xorv^len)<=len){
            int moku=len-(xorv^len);
            vp p=gx[fi];
            int wa=0,ac=m;
            while(ac-wa>1){
                int wj=(wa+ac)/2;
            ...
            cout <<fi << " " <<  0 << " " << fi << " " << ac << endl;
            return;
        }
    }
    for(auto [fi,len]:leny){
        if((xorv^len)<=len){
            int moku=len-(xorv^len);
            vp p=gy[fi];
            int wa=0,ac=n;
