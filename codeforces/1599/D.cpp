...
#line 2 "/home/maspy/compro/library/geo/dynamicupperhull.hpp"
...
#line 2 "/home/maspy/compro/library/geo/convex_layers.hpp"
...
#line 3 "/home/maspy/compro/library/geo/convex_polygon.hpp"
...
void solve() {
  ...
  int L = len(layer);
  // for (auto& x: layer) SHOW(x);
  vc<pair<int, int>> pos(N);
  FOR(i, L) FOR(j, len(layer[i])) pos[layer[i][j]] = {i, j};

  Doubling<Monoid_Add<int>, 20> G(N);
  FOR(i, L - 1) {
    vc<int> I = layer[i], J = layer[i + 1];
    int N1 = len(I), N2 = len(J);
    FOR(i, 2 * N2 + 10) J.eb(J[i]);
    vc<int> nxt(N1);
    ...
    FOR(k, N1) {
      ...
      auto check = [&](int j) -> bool {
        assert(1 <= j && j + 1 < len(J));
        P x = A[J[j - 1]], y = A[J[j]], z = A[J[j + 1]];
        return (ccw(a, x, y) <= 0 && ccw(a, y, z) >= 0);
      };
      while (!check(p)) ++p;
      nxt[k] = J[p];
    }
  ...
  G.build();
  vc<int> I = layer[0];
  vc<P> X = rearrange(A, I);
  ConvexPolygon<ll> poly(X, 1);
  ...
  FOR(Q) {
    ...
    --K;
    // first vertex
    auto [v, dot] = poly.max_dot(P(b, -a));
    v = I[v];
    // print("max_dot", v, dot);
    int k = G.max_step([&](auto e) -> bool { return e <= K; }, v);
    auto [w, cost] = G.calc(v, k);
    // print("goto", v, w, cost);
    assert(cost <= K);
    K -= cost, v = w;
    auto [i, j] = pos[v];
