...
void solve(int test){
    ...
    cntgr[1]+=x;
    int gr=0;
    repi(i,1,30)if(cntgr[i]%2)gr^=memogr[i];
    if(gr)cout << "Vasya" << endl;
...
/*
    map<vi,int> seen;
    map<vi,int> memo;
    auto gr=[&](auto gr,vi a)->int{
        if(seen[a])return memo[a];
        ...
        rep(i,a.size()){
            int x=a[i];
            vi next;
            rep(i,a.size())if(a[i]%x)next.push_back(a[i]);
            vs.push_back(gr(gr,next));
        }
        memo[a]=mex(vs);
        return memo[a];
    };
