...
void solve()
{
        ll a,b,kx,ky,qx,qy;
    cin>>a>>b>>kx>>ky>>qx>>qy;
    vector<pair<ll,ll>>queen(8);
    vector<pair<ll,ll>>king(8);
    queen[0]={kx-a,ky-b};
    queen[1]={kx-a,ky+b};
    queen[2]={kx+a,ky-b};
    queen[3]={kx+a,ky+b};
    ...
    for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
            if(queen[i]==king[j]){
                //if pair not found in ans vector then push it
                if(find(ans.begin(),ans.end(),queen[i])==ans.end()){
                    ans.push_back(queen[i]);
                }
            }
        }
    }
