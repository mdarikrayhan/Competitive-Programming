...
int get(int l1, int r1, int l2, int r2){
	...
	if(r2 <= r1) return r2 - l2;
	if(l2 <= r1) return r1 - l2;
	return 0;
}
...
void solve() {
	...
	if(max(l1, l2) <= min(r1, r2)){		
		int need = max(0LL, k - n * (min(r1, r2) - max(l1, l2)));
		int can = n * (abs(l1 - l2) + abs(r1 - r2));

		cout << min(can, need) + max(0LL, need - can) * 2 << endl;
	}
	else{
		int inter = max(l1, l2) - min(r1, r2), res = inf;
		for(int i = 1; i <= n; ++i){
			int curr = inter * i;
			int can = i * (max(r1, r2) - min(l1, l2));
			cmin(res, curr + min(can, k) + max(0LL, k - can) * 2);
		}
		cout << res << endl;
	}
