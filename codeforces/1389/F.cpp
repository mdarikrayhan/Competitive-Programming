...
struct SegTree
{
    ...
    void propagate(ll ver,ll L,ll R)        //update done only on ex
    {
        ...
        st[ver].mn+=lazy[ver],st[ver].mx+=lazy[ver],st[ver].ex+=lazy[ver]; // during lazy propagation you have to chg this line according to the condition
    ...
    void pUpdate(ll ver,ll L,ll R,ll pos,ll val)
    {
        ...
        if(L==R)
        {
            st[ver].dp=max(val, st[ver].dp);
            st[ver].mn = st[ver].mx = st[ver].dp + st[ver].ex;
            return;
        }
        ll M=(L+R)/2;
...
int main()
{
    ...
    vector<array<int, 3>> s(n);        //{l, r, t}
    for(int i = 0; i < n; i ++) cin >> s[i][0] >> s[i][1] >> s[i][2], store.insert(s[i][0]), store.insert(s[i][1]);

    int c = 1;
    map<int, int> cmp;
    for(auto x : store) cmp[x] = ++ c;

    for(auto &x : s)    x[0] = cmp[x[0]], x[1] = cmp[x[1]];

    sort(all(s), [&](array<int, 3> a, array<int, 3> b) {return mp(a[1], a[0]) < mp(b[1], b[0]);});

    red.init(2 * n + 1), blue.init(2 * n + 1);
    ...
    for(int i = 0; i < n; i ++)
    {
        if(s[i][2] == 1)            //red
        {
            ...
            blue.update(1, s[i][0] - 1, 1);
            int opt = blue.query(1, s[i][0] - 1).mx;
            ans = max(ans, opt);
            red.update(s[i][1], max(opt, r));
        }
        else
        {
            b ++;
            red.update(1, s[i][0] - 1, 1);
            int opt = red.query(1, s[i][0] - 1).mx;
            ans = max(ans, opt);
            blue.update(s[i][1], max(opt, b));
        }
    }
    ans = max(ans, max(r, b));
    cout << ans << endl;    
