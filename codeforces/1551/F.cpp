...
void _solve(){
    ...
    if(k == 2) preturn(n * (n-1) / 2);
    ll ans = 0;
    vi cnt;
    auto dfs = yf([&](auto self, int u, int p, int d) -> void {
        cnt[d]++;
        for(auto to: g[u]) if(to != p) self(to, u, d+1);
    });
    forn(r, n){
        auto ways = vv(n, k+1, 0LL); fornn(i, 1, n) ways[i][0] = 1;
        for(auto x: g[r]){
            cnt.assign(n, 0);
            dfs(x, r, 1);
            fornn(d, 1, n){
                if(cnt[d] == 0) break;
                // forbn(j, 1, k+1) ways[i][j] = (ways[i][j] + ways[i][j-1] * cnt[i] % mod) % mod;
                // forb(j, k) ways[d][j+1] = (ways[d][j+1] + ways[d][j] * cnt[d]) % mod;
                forb(j, k) {
                    ways[d][j+1] += ways[d][j] * cnt[d];
                    ways[d][j+1] %= mod;
                }
            }
        }
        vll v; fornn(i, 1, n) if(ways[i][k]) v.pb(ways[i][k]); debug2(r, v);
        fornn(i, 1, n) ans = (ans + ways[i][k]) % mod;
    }
    out(ans);
}
