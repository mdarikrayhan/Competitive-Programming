...
constexpr bool MultiCase=true;

constexpr long long K=128;
long long x,y,u,v,m;
long long A[K],B[K],dp[K][K][K];
inline void dfs(long long i,long long x,long long y,long long dep){
    if(~dp[i][x][y]&&dp[i][x][y]<=dep)return void();
    dp[i][x][y]=dep;
    dfs(i,x&y,y,dep+1);
    dfs(i,x|y,y,dep+1);
    dfs(i,x,y^x,dep+1);
...
inline int SolveMain([[maybe_unused]]long long TC){
    ...
    v=read();
    m=read();
    register long long a=0,b=0,c=0;
    for(register int i=0;i<=29;++i){
        register long long coef=0;
        x>>i&1?coef|=1:0;
        y>>i&1?coef|=2:0;
        m>>i&1?coef|=4:0;
        a|=1<<coef;A[i]=coef;
    ...
    for(register int i=0;i<=29;++i){
        register long long coef=0;
        u>>i&1?coef|=1,b|=1<<A[i]:0;
        v>>i&1?coef|=2,c|=1<<A[i]:0;
        B[i]=coef;
    }
    if((b&1)|(c&1))return !write(-1);
    for(register int i=0;i<=29;++i){
        for(register int j=i+1;j<=29;++j){
            if(A[i]==A[j]&&B[i]!=B[j])return !write(-1);
        }
    }
    write(dp[a>>1][b>>1][c>>1]);
    return 0;
}
