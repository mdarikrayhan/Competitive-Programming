...
//Problem Name  :
//Discription   :
//Reference from: http://codeforces.com/contest/222/submission/2161226
//==================================================================
...
int n, m, sieve[MAXN], A[N][25], B[N][26];
int divA[MAXN], divB[MAXN], pr[MAXN];
...
void getSieve(){
    ...
    forn (i, 2, MAXN) {
        ...
        for (int j = 0; j < cnt && pr[j] <= sieve[i] && i * pr[j] < MAXN; j++)
            sieve[i * pr[j]] = pr[j];
    }
}
...
int main(){
    ...
    pin(n), space, pin(m), endline;

    forn (i, 0, n){
        val = 1;
        for (int j = 0; A[i][j]; j++)
            if (divB[A[i][j]]) divB[A[i][j]]--;
            else val *= A[i][j];
        pin(val), space;
    }
