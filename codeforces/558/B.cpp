...
int main()
{
    ...
    int maxFreq = 0, minSubArrayLength = n + 1, ansStartId, ansEndId;
    for (int i = 1; i <= 4; ++i)
    {
        dbga(freq[i]);
    }
    for (int i = 1; i <= 1e6; ++i)
    {
        if (freq[i])
        {
            ...
            {
                maxFreq = freq[i];
                minSubArrayLength = endId[i] - startId[i] + 1;
                ansStartId = startId[i] + 1;
                ansEndId = endId[i] + 1;
            }
            else if (freq[i] == maxFreq)
            {
                int prevSubArrayLength = minSubArrayLength;
                minSubArrayLength = min(minSubArrayLength, endId[i] - startId[i] + 1);
                if (minSubArrayLength < prevSubArrayLength)
                {
                    ansStartId = startId[i] + 1;
                    ansEndId = endId[i] + 1;
                }
