...
{
    vector<int> par;
    vector<deque<pair<int, int>>> keep;
    void init(int n)
    {
        par.resize(n + 1);
        keep.resize(n + 1);
        for(int i = 1; i <= n; i ++)    par[i] = i, keep[i].clear();
    }
...
int32_t main()
{
    ...
    while(tt--)
    {
        ...
        function<void(int, int)> dfs = [&](int u, int p)
        {
            ...
            for(auto v : adj[u])
                if(v != p and can)
                    dfs(v, u), store[dp[v]].pb(v);
            
            if(u == 1)  return;

            int start = 1 - (((int)adj[u].size()) % 2);
            int e = store[0].size(), o = store[1].size();
            ...
            if(e == o)
            {
                ...
                int rep = store[start].front();

                for(int j = 0; j < e; j ++)
                {
                    dsu.unite(rep, store[start][j]);
                    dsu.unite(rep, store[start ^ 1][j]);
                }
                dsu.unite(rep, u);
            }
            else if(abs(e - o) == 1)
            ...
            else if(abs(e - o) == 2)
            {
                ...
                int rep = store[start].front();
                for(int j = 0; j < min(e, o) + 1; j ++)
                {
                    dsu.unite(rep, store[start][j]);
                    dsu.unite(rep, store[start ^ 1][j]);
                }
                dsu.unite(rep, store[start].back());
            }
            else
            {
                can = false;
        ...
        if(can)
        {
            ...
            if(e == o)
            {
                ...
                for(int i = 0; i < e; i ++)
                {
                    dsu.unite(rep, store[start][i]);
                    dsu.unite(rep, store[start ^ 1][i]);
                }
            }
            else if(abs(e - o) == 1)
            {
        ...
        for(auto edge : dsu.keep[dsu.get(1)])
            cout << edge.first << sp << edge.second << endl;
    }
}
