...
int ceil_div(ll x, ll y){return (x + y - 1) / y;}
...
const int MAXN = 2e5+5, R = 2000;
int n, m;
vector<vector<array<int, 3>>> q(MAXN, vector<array<int, 3>> (2));
...
int main()
{
    ...
    for(int b = 1; b <= ceil_div(m, R); b ++)
    {
        set<pair<int, int>> doubt;

        for(int i = (b - 1) * R + 1, t, x, y; i <= min(m, b * R); i ++)         //input edges of this block, remove past edges which are now candidates for deletion, and add later into the compressed graph
        {
            cin >> t >> x >> y;
            q[i][0][0] = q[i][1][0] = t;
            q[i][0][1] = ((x + 0 - 1) % n) + 1, q[i][0][2] = ((y + 0 - 1) % n) + 1;
            q[i][1][1] = ((x + 1 - 1) % n) + 1, q[i][1][2] = ((y + 1 - 1) % n) + 1;
            if(q[i][0][1] > q[i][0][2]) swap(q[i][0][1], q[i][0][2]);
            if(q[i][1][1] > q[i][1][2]) swap(q[i][1][1], q[i][1][2]);

            if(t == 1)
            {
                auto pp = mp(q[i][0][1], q[i][0][2]);

                if(edge.count(pp))  edge.erase(pp), doubt.insert(pp);

                pp = mp(q[i][1][1], q[i][1][2]);
        ...
        for(auto xy : edge) dsu.unite(xy.first, xy.second);     //dsu unitions (is that a word???) using the safe edges

        vector<pair<int, pair<int, int>>> adj[n + 1];       //gotta take care of multiple edges between dsu components 
        for(auto xy : doubt)    adj[dsu.get(xy.first)].pb(mp(dsu.get(xy.second), mp(xy.first, xy.second))), adj[dsu.get(xy.second)].pb(mp(dsu.get(xy.first), mp(xy.first, xy.second)));

        for(int i = (b - 1) * R + 1, t, x, y; i <= min(m, b * R); i ++)
        {
            t = q[i][last][0], x = q[i][last][1], y = q[i][last][2];

            if(t == 1)
            {
                if(doubt.count(mp(x, y)))
                {
                    // remove
                    doubt.erase(mp(x, y));

                    auto bad = mp(dsu.get(y), mp(x, y));
                    auto it = adj[dsu.get(x)].begin();

                    while(*it != bad)   it ++;
                    adj[dsu.get(x)].erase(it);


                    bad = mp(dsu.get(x), mp(x, y));
                    it = adj[dsu.get(y)].begin();
                    while(*it != bad)   it ++;
                    adj[dsu.get(y)].erase(it);
                }
                else
                {
                    // add
                    doubt.insert(mp(x, y));
                    adj[dsu.get(x)].pb(mp(dsu.get(y), mp(x, y)));
                    adj[dsu.get(y)].pb(mp(dsu.get(x), mp(x, y)));
                }
            }
            else
            {
                ...
                last = vis.count(dsu.get(y));
                ans += to_string(last);
            }
        }
        for(auto xy : doubt)    edge.insert(xy);
    }
    cout << ans << endl;
}
