...
void solve() { string s, ans; ll n, num = 0; cin >> n; unordered_map<char, int> freq; for (char i = 'a'; i <= 'z'; i++) { freq[i] = 0; } rep(i, 0, n) { cin >> num; for (auto j : freq) { if (j.second == num) { s += j.first; freq[j.first]++; break; } } } cout << s << nl; }
