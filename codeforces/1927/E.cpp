...
void solve()
{
    ll n, x;cin >> n >> x;vector<ll> ans(n), pre(x);pre[0] = n, pre[1] = 1;
    rep(i,0,x){ll maxi = 0, mini = n + 1, sum = 0, temp;if (i & 1){temp = 1;}else{temp = -1;}
        for (int j = i; j < n; j += x){
            ans[j] = pre[i] + (temp * (sum++));
            maxi = max(ans[j], maxi);
            mini = min(ans[j], mini);}
        if (temp == -1 && (i + 2 < x)){pre[i + 2] = mini - 1;}
        else if (i + 2 < x){ pre[i + 2] = maxi + 1;}
    }
    rep(i,0,n)
    {
        cout << ans[i] << " ";
