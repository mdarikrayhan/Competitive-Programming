...
ll evaluate_string(string &s){
    ...
    for (ll i = 0; i < len; i++) {
        ...
        if (!isdigit(currentChar) && !iswspace(currentChar) || i == len - 1) {
            if(neg) currentNumber*=-1;
            if (operation == '*') {
                ll stackTop = stack.top();
                stack.pop();
            ...
            else stack.push(currentNumber);
            operation = currentChar;
            if(operation == '-') neg=true;
            else if(i<(len-1)){
                if(s[i+1]=='-'){
                    neg=true;
                    i+=1;
...
void stack_push(stack <char> &st, string &s){
    for(int i=0;i<s.length();i+=1) st.push(s[i]);
}
...
{   
    ...
    while(t_t--){
        ...
        rep(i,n){
            rep(j,n){
                if(i==0) pf[i][j]=dp[i][j];
                else pf[i][j]=pf[i-1][j]+dp[i][j];
            }
        }
