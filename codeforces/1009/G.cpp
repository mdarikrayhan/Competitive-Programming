...
inline bool check() {
    for (int i = 1; i < 64; i++) {
        ...
        for (int j = 0; j < 6; j++)
            if (i >> j & 1) res += cnt[j];
        if (res > out[i]) return 0;
    }
    return 1;
}
int main() {
    read_cstr(str + 1), n = strlen(str + 1);
    ...
    for (int i = 1; i <= m; i++) {
        ...
        string strx;
        read(pos, strx);
        for (char v : strx) res |= 1 << (v - 'a');
        ins(res, 1), rx[pos] = res;
    }

    for (int i = 1; i <= n; i++)
        if (!rx[i]) rx[i] = 63, ins(rx[i], 1);
    if (!check()) {
        return println("Impossible"s), 0;
    }
    for (int i = 1; i <= n; i++) {
        for (char c = 'a'; c <= 'f'; c++) {
            if ((rx[i] >> (c - 'a')) & 1 ^ 1) continue;
            ins(rx[i], -1), --cnt[c - 'a'];
            if (check()) {
                pc(c);
                break;
