...
#line 2 "/home/maspy/compro/library/mod/dynamic_modint.hpp"
...
#line 5 "/home/maspy/compro/library/mod/mod_pow.hpp"
...
#line 6 "/home/maspy/compro/library/mod/primitive_root.hpp"
...
#line 6 "/home/maspy/compro/library/mod/dynamic_modint.hpp"
...
void solve() {
  ...
  vc<int> rest(N, 1);
  vc<int> pre(N), nxt(N);
  FOR(i, N) pre[i] = i - 1, nxt[i] = i + 1;

  vc<mint> fact(N + 1);
  fact[0] = 1;
  FOR(i, N) fact[i + 1] = fact[i] * (i + 1);

  map<tuple<int, int, int>, int> MP;

  int good = N - 1; // 残っているやつの中だけで (x,x+1)
  int base = 0;
  vc<mint> ANS(N + 1);
  ...
  FOR(i, N) {
    ...
    FOR(x, a) {
      ...
      if (R < N && R == x + 1) --m;
      // 直後に置けるやつがあるか
      if (x + 1 < N && nxt[x] == x + 1) ++m;
      X[i] = x;
      tuple<int, int, int> t = {k, N - 1 - i, m};
      MP[t]++;
    }
    if (a == last + 1) ++base;
    int L = pre[a], R = nxt[a];
  ...
  //   auto [k, n, m] = key;
  //   vc<mint> f(m + 1);
  //   FOR(i, m + 1) f[i] = fact[n - i] * C_dense<mint>(m, i);
  //   g(x)=f(x-1)
  //   vc<mint> g(m + 1);
  //   FOR(i, m + 1) FOR(j, i, m + 1) {
  //     g[i] += mint(-1).pow(j - i) * C_dense<mint>(j, i) * f[j];
  //   }
  //   // vc<mint> g = poly_taylor_shift<mint>(f, -1);
  //   FOR(i, m + 1) ANS[k + i] += mint(cnt) * g[i];
  // }
  // c(x-1)^ix^j の形の和で書ける
  vv(mint, F, N + 1, N + 1);
  for (auto& [key, cnt]: MP) {
    auto [k, n, m] = key;
    FOR(i, m + 1) {
      mint x = fact[n - i] * C_dense<mint>(m, i);
      F[i][k] += x * mint(cnt);
    }
  }
  // sum F[i][j](x-1)^ix^j
