        ...
        multicase
    {
        ...
        if (sum & 1)
        {
            ...
            for (auto val : odd)
            {
                ...
                while (temporary % 2 != 0)
                {
                    temporary = floor(temporary / 2);
                    ops++;
                }
                oddallstep.push_back(ops);
            }
            sort(all(oddallstep));
            oddminstep = oddallstep[0];

            int evenminstep = INT_MAX;
            if ((int)even.size() > 0)
            {
                vl evenallstep;
                for (auto val : even)
                {
                    ...
                    evenallstep.push_back(ops);
                }
                sort(all(evenallstep));
                evenminstep = evenallstep[0];
            }
            cout << min(oddminstep, evenminstep) << nl;
        }
        else
        {
            cout << 0 << nl;
