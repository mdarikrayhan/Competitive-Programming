...
T2の葉aをひとつとる
T1のパスabから辺acとマッチ
辺acを縮約したもの：大きさが1小さくなった状態
*/

void solve() {
  LL(N);
  Graph<int, 0> G1(N), G2(N);
  G1.read_tree();
  G2.read_tree();
  ...
  Tree_Monoid<decltype(T1), Monoid_Add<int>, true> TM(
      T1, [&](int i) -> int { return 1; });

  FOR_R(i, 1, N) {
    int a = T2.V[i];
    int b = T2.parent[a];
    // a->b ではじめて生き残っている辺
    int c = TM.max_path([&](auto e) -> bool { return e == 0; }, a, b);
    int d = T1.jump(c, b, 1);
    ANS.eb(c, d, a, b);
    int idx = T1.get_eid(c, d);
    TM.set(idx, 0);
  }
