...
template <typename T> vector<T> getPre(vector<T> &v) {
  vector<T> pre = v;
  re(i, 1, sz(pre)) pre[i] += pre[i-1];
  return pre;
}

template <typename T>
T preSum(vec<T> &v, int l, int r) {
  if (l >= sz(v) || r < 0) return 0;
  return v[min(sz(v)-1, r)] - (l <= 0 ? 0 : v[l-1]);
};
...
template <typename A> struct matrix {
  int n, m;
  vector<vector<A>> v;
  ...
  matrix(int _n, int _m, int initial_value=0): n(_n), m(_m) {
    v.assign(n, vector<A>(m, initial_value));
  }

  matrix(vector<vector<A>> _v): v(_v) {
    n = sz(v); m = sz(v[0]);
  }

  vector<A>& operator[](int i) { return v[i]; };

  matrix operator + (const matrix& o) const {
    assert(n == o.n && m == o.m);
    matrix res(n, m);
    re(i, n) re(j, m) res.v[i][j] = v[i][j] + o.v[i][j];
    return res;
  }
...
void solve() {
  ...
  re(i, n) {
    re(j, n) {
      re(l, sz(a[i])) {
        ...
        int jind = 0;
        int steps = 0;
        while (iind < sz(a[i]) && jind < sz(a[j]) && a[i][iind] == a[j][jind]) {
          iind++;
          jind++;
          steps++;
        }
        if (iind == sz(a[i])) {
          assert(steps > 0);
          A[ind[{j, jind, steps-1}]][ind[{i, l, 0}]]++;
        } else if (jind == sz(a[j])) {
          assert(steps > 0);
          A[ind[{i, iind, steps-1}]][ind[{i, l, 0}]]++;
        }
      }
    }
  }
  ...
  re(i, n) {
    re(j, n) {
      ...
      if (iind == sz(a[i])) {
        assert(steps > 0);
        // dbg(i, j, jind, steps-1);
        re(x, n) {
          A[ind[{j, jind, steps-1}]][ind[{x, sz(a[x]), 0}]]++;
        }
      } else if (jind == sz(a[j])) {
        assert(steps > 0);
        // dbg(j, i, iind, steps-1);
        re(x, n) {
          A[ind[{i, iind, steps-1}]][ind[{x, sz(a[x]), 0}]]++;
        }
      }
    }
  }
  ...
  re(i, n) {
    re(j, sz(a[i])+1) {
      re(k, j) {
        // A[ind[{i, j, k+1}]][ind[{i, j, k}]]++;
        A[ind[{i, j, k}]][ind[{i, j, k+1}]]++;
      }
    }
  }

  matrix<mi> x(M, 1);
  x[ind[{0, sz(a[0]), 0}]][0] = 1;
  // dbg(ind[{0, sz(a[0]), 0}]);

  auto Apow = A.pow(m);
  auto y = Apow * x;
  mi ans = 0;
  re(i, n) ans += y[ind[{i, sz(a[i]), 0}]][0];
  dbg(ans);
  cout << ans << '\n';
}
