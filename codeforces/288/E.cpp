...
vll anss;
mint dp[1000005][2][2];
mint dpsum[1000005][2][2];
mint dpsum2[1000005][2][2];
void solve(int test){
    string s,t; cin >> s >> t;
    int n=s.size();
    vi a(n),b(n);
    ...
    auto f=[&](int i){
        if(7>b[i])return false;
        if(7<b[i])return true;
        if(i==n-1)return true;
        return minv[i+1]>=4;
    };
    auto g=[&](int i){
        if(4<a[i])return false;
        if(4>a[i])return true;
        if(i==n-1)return true;
        return maxv[i+1]<=7;
    ...
    rep(i,n)rep(j,2)rep(k,2){
        mint now=dp[i][j][k];
        mint nowsum=dpsum[i][j][k];
        mint nowsum2=dpsum2[i][j][k];
        if(now.val()==0)continue;
        for(auto next:di){
            if(!j && next>b[i])continue;
            if(!k && next<a[i])continue;
            int nj=j;
            int nk=k;
            if(next<b[i])nj=1;
            if(next>a[i])nk=1;
            dp[i+1][nj][nk]+=now;
            dpsum[i+1][nj][nk]+=nowsum*10+next*now;
            dpsum2[i+1][nj][nk]+=nowsum2*100+20*nowsum*next+next*next*now;
        }
        {
            int ok=1;
            if(!j && !f(i))ok=0;
            ...
            if(ok){
                mint num=dp[i][j][k];
                mint x=dpsum[i][j][k];
                mint x2=dpsum2[i][j][k];
                int keta=n-i;
                mint base=mint(pow_pow(10,keta-1,mod)-1)/9;
                mint bx=base*4+pow_pow(10,keta-1,mod)*7;
                mint by=base*7+pow_pow(10,keta-1,mod)*4;

                mint sumx=x*pow_pow(10,keta,mod);
                mint sumx2=x2*pow_pow(10,keta*2,mod);

                ans+=sumx2+bx*sumx+sumx*by+bx*by*num;
            }
        }
    }
    cout << ans.val() << endl;
