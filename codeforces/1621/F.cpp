    ...
    while(tt--)
    {
        ...
        if(doable[1] >= doable[2])
        {
            while(doable[2] > 0)
            {
                ...
                if(!window.empty())
                {
                    int opt = window.top() - 1;
                    window.pop();

                    if(opt > 0) window.push(opt);
                    else    ++ doable[3];
                }
            }
...
/*
obs: 
1) number of 1-moves constant
2) never optimal to do 3-move if 2-move available, as performing a 2-move may increase number of 3-moves but never vice versa
3) performing a 3-move increases doable 2-moves

cases:
1) (1-moves = 2-moves)
    here first we can do like 1 2 1 2 1 2 and then if
        a) b > c do 3 2 3 2 3 2 3 2
        b) b <= c do nothing
2) (1-moves < 2-moves)
    here first we can do like 2 1 2 1 2 1 2 by greedily picking from smallest window such that generated 3-moves are maximised
        a) b > c do 3 2 3 2 3 2 3 2, if there are any at edges, and also some remaining 2-moves then perform these as well
        b) b <= c do nothing
3) (1-moves > 2-moves)
    here first we can do like 1 2 1 2 1 2 1 2 by greedily picking from smallest window such that generated 3-moves are maximised and all 2-moves are exhausted
    now we can only perform some 3-2 combos and then end this shit with a final 1 move
        a) b > c perform 3 2 3 2 3 2 3 2 and end with a 1 move
        b) b <= c do nothing
*/
