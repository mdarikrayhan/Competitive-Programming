...
struct Dinic {
  ...
  Dinic(int _n) : n(_n), lvl(n), ptr(n), q(n), adj(n) {}
  void clear() {
    lvl.assign(n, 0); ptr.assign(n, 0); q.assign(n, 0);
    ea(v, adj) v.clear();
  }
	...
	ll calc(int s, int t) {
		ll flow = 0; q[0] = s;
		re(L,0,31) do { // 'int L=30' maybe faster for random data
      lvl.assign(n, 0); ptr.assign(n, 0);
			int qp = 0, qe = lvl[s] = 1;
			while (qp < qe && !lvl[t]) {
				int v = q[qp++];
				for (Edge e : adj[v])
			...
			if (lvl[t]) while (ll p = dfs(s, t, LLONG_MAX)) flow += p;
		} while (lvl[t]);
		return flow;
	}
  // return the positive flows in the form of tuples {u, v, f}, given that calc() has been called.
  // Interpretation: there are f units of flow from source u to destination v.
  // warning: does not respect the way edges were added.
  vector<tuple<int, int, ll>> flows() {
    vec<tuple<int, int, ll>> res;
    re(u, sz(adj)) {
      ea(e, adj[u]) {
        if (e.flow()) {
          res.pb({u, e.to, e.flow()});
        }
      }
    }
    return res;
...
ll solve(int n, vi a, vl b) {
  ...
  re(i, n) {
    set<int> found;
    rer(j, i) {
      if (a[i] % a[j] == 0 && !found.count(a[j])) {
        dinic.addEdge(i, j, e9);
        found.ins(a[j]);
      }
    }
  }
...
ll attempt(int n, vi &a, vl &b) {
  int msk = rint(0, p2(n)-1);
  re(i, n) {
    if (test(msk, i)) {
      re(j, i) {
        if (a[i] % a[j] == 0 && !test(msk, j)) {
          return 0;
        }
      }
    }
  ...
  // if (tot == 1) dbg(tot, ll_to_binary(msk), b);
  // if (tot == 1) dbg(ll_to_binary(msk));
  return tot;
}


ll brute(int n, vi a, vl b) {
  ll mx = 0;
  re(i, 10000) ckmax(mx, attempt(n, a, b));
  return mx;
}
...
void sol() {
  ...
  //   int n = rint(2, 8);
  //   vi a(n);
  //   re(i, n) a[i] = rint(1, 20);
  //   vl b(n);
  //   re(i, n) b[i] = rint(-10, 10);
