...
struct Poly:public std::vector<MInt<P>>{
    ...
    constexpr Poly shift(int k)const{
        if(k>=0){
            ...
            return b;
        }else if(this->size()<=-k) return Poly();
    ...
    constexpr Poly integr()const{
        Poly res(this->size()+1);
        for(int i=0;i<this->size();++i) res[i+1]=(*this)[i]/(i+1);
        return res;
    }
...
Poly<P>berlekampMassey(const Poly<P>&s){
    ...
    for(int i=0;i<s.size();i++){
        auto delta=s[i];
        for(int j=1;j<=c.size();j++) delta-=c[j-1]*s[i-j];
        if(delta==0) continue;
        if(f==-1){
            c.resize(i+1);
            f=i;
            ...
            d.insert(d.begin(),1);
            MInt<P>df1=0;
            for(int j=1;j<=d.size();j++)df1+=d[j-1]*s[f+1-j];
            assert(df1!=0);
            auto coef=delta/df1;
            d*=coef;
            Poly<P>zeros(i-f-1);
...
MInt<P>linearRecurrence(Poly<P>p,Poly<P>q,i64 n){
    ...
    while(n>0){
        ...
        auto newp=p*newq;
        newq=q*newq;
        for(int i=0;i<m;i++) p[i]=newp[i*2+n%2];
...
template<int P=1000003>
Poly<P>get_poly_by_points(const std::vector<std::pair<Z,Z>>&p){
	int n=p.size();
	std::vector<Z>L(n),l(n+1);
	l[0]=1;
	for(int i=0;i<n;++i){
		for(int j=i;j>=0;--j){
			l[j+1]=l[j+1]+l[j];
	...
	for(int i=0;i<n;++i){
		...
		Z w=0;
		for(int j=n-1;j>=0;--j){
			a[j]=l[j+1]+p[i].first*a[j+1];
			w=w*p[i].first+a[j];
		}
		w=w.inv();
		for(int j=0;j<n;++j){
			L[j]=L[j]+a[j]*p[i].second*w;
		}
	}
	return Poly(L);
}
...
void solve(){
    std::vector<std::pair<Z,Z>>p;
    for(int i=0;i<=10;++i){
        std::cout<<"? "<<i<<std::endl;
		int y; std::cin>>y;
		p.push_back({i,y});
	}
	auto f=get_poly_by_points(p);
	for(int i=0;i<P;++i){
		Z ans=0;
		for(int j=10;j>=0;j--){
			ans=ans*i+f[j];
