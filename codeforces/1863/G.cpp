...
int n;
vector<int> a(MAXN, 0), s(MAXN, 0), d(MAXN, 0);
vector<Mint> dp(MAXN, 1), ex(MAXN, 1);

vector<bool> vis(MAXN, false), vis2(MAXN, false);

vector<bool> in_c(MAXN, false);
vector<int> adj(MAXN), g[MAXN], r[MAXN];
...
int32_t main()
{
    ...
    for(int i = 1; i <= n; i ++)    cin >> a[i], ++ s[a[i]], adj[i] = a[i], g[i].pb(a[i]), g[a[i]].pb(i), r[a[i]].pb(i), d[i] |= (i == a[i]);
    ...
    for(int i = 1; i <= n; i ++)    if(!vis2[i])
    {
        ...
        //process of altering tree can be seen as:
        // perform all incycle operations first, 
        // fuck around with subtrees of cycle nodes (choose non intersecting vertical paths) (root might go down)
        ...
        {
            //lets find that if some node is not satisfied by anything in the cycle, then how many ways can its subtree be arranged (with the "arrived" node potentially going inside it)
            Mint extra = 1;
            ...
            for(auto c : cycle)
            {
                ...
                //now, node is:
                //a) satisfied in cycle
                //b) not satisfied in cycle:
                //      1) doesnt link with shit below
                //      2) links with one of the things below

                dp[c] += sub;
                dp[c] += sub;
                dp[c] += (((int)r[c].size() - 1) * sub);
