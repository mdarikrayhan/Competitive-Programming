...
void solve(int test){
    ...
    vvi g2(n);
    rep(i,m)if(!uf.issame(u[i],v[i])){
        g2[uf.root(u[i])].push_back(uf.root(v[i]));
        g2[uf.root(v[i])].push_back(uf.root(u[i]));
    }
    rep(i,n){
        sort(all(g2[i]));
