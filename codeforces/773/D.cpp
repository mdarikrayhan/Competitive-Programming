...
bool cmp(const pair<int, int> &a, const pair<int, int> &b)  {return a.S < b.S;}
...
int main()
{
    ...
    for(ll i = 1, w; i < n; i ++)
        for(ll j = i + 1; j <= n; j ++)    
            cin >> w, adj[i].pb({j, w}), adj[j].pb({i, w}), best = min(best, w);
    
    for(ll u = 1; u <= n; u ++)
    {
        for(auto &v : adj[u])
            v.S -= best, mn[u] = min(mn[u], v.S), mn[v.F] = min(mn[v.F], v.S);
        sort(all(adj[u]), cmp);
    }
    ...
    for(int s = 1; s <= n; s ++)
    {
        ...
        set<pair<ll, pair<ll, ll>>> q;    //{d, {l, u}}

        vector<bool> mark(n + 1, 0);
        vector<ll> dis(n + 1, 0), las(n + 1, 0);
        mark[s] = 1;

    
        for(auto v : adj[s])    q.insert({v.S, {v.S, v.F}}), dis[v.F] = v.S, las[v.F] = v.S;
        ...
        while(!q.empty())
        {
            ...
            ll d = cur.F, last = cur.S.F, u = cur.S.S;
            ...
            for(auto v : adj[u])
            {
                ...
                if(!mark[v.F])
                {
                    if(d + v.S <= dis[v.F] and d + v.S < ans)
                    {
                        q.erase({dis[v.F], {las[v.F], v.F}});
                        dis[v.F] = d + v.S, las[v.F] = v.S;
                        q.insert({dis[v.F], {las[v.F], v.F}});
                    }
                }
            }
        }
