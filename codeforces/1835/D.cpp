...
int T,n,m,a[N],d[N],sum,cnt,b[N],bt;
int dfn[N],low[N],dlt,stk[N],tp,inc[N];
struct Eg{int x,y;}g[N];
vt lk[N];
ll K,ans;
void tarjan(int x){
    dfn[x]=low[x]=++dlt,stk[++tp]=x;
    for(int y:lk[x])
        if(!dfn[y])tarjan(y),low[x]=min(low[x],low[y]);
        else if(!inc[y])low[x]=min(low[x],dfn[y]);
    if(dfn[x]==low[x]){
        do inc[stk[tp]]=x;
        while(stk[tp--]!=x);
    }
...
void dfs(int x){
    vs[x]=1,b[++bt]=d[x];
    for(int y:lk[x])
        if(vs[y])sum=__gcd(sum,abs(d[x]+1-d[y]));
        else d[y]=d[x]+1,dfs(y);
}
int main(){
    ...
    for(i=1;i<=m;++i)
        if(inc[g[i].x]==inc[g[i].y])
            lk[g[i].x].push_back(g[i].y);
    for(x=1;x<=n;++x)
        if(inc[x]==x){
            sum=bt=0,dfs(x);
            if(!sum)continue;
            ...
            if((r+r)%sum)continue;
            if(!r){
                for(i=1;i<=bt;++i)++a[b[i]%sum];
                for(i=0;i<sum;++i)
                    ans+=(1ll+a[i])*a[i]/2,a[i]=0;
                continue;
            }
            for(i=1;i<=bt;++i)++a[b[i]%sum];
            for(i=0;i<r;++i)
