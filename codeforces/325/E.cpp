...
void solve() {
  ...
  if (N == 2) return print(0, 1, 0);
  if (N == 4) return print(0, 1, 3, 2, 0);
  ...
  G.build();
  auto [A, es] = euler_walk<decltype(G)>(G, 0);
  if (A.empty()) return print(-1); // ないかも

  FOR(i, 2 * n) {
    // to に正しいラベルをつける
    int a = 2 * A[i] + 0, b = 2 * A[i] + 1;
    a %= N, b %= N;
    if ((A[i + 1] - a) % n == 0) A[i + 1] = a;
    elif ((A[i + 1] - b) % n == 0) A[i + 1] = b;
    else {
      assert(0);
    }
  }
