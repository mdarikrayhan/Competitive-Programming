    ...
    if(n>=8){
        ...
        if(st.size()){
            ...
            while(it!=st.end()){
                int v=*it;it++;
                rep(i,vs.size()){
                    if(!mp[P(vs[i],v)] && !mp[P(vs[i+1],v)]){
                        vi next;
                        rep(j,i+1)next.push_back(vs[j]);
                        next.push_back(v);
                        repi(j,i+1,vs.size())next.push_back(vs[j]);
                        swap(vs,next);
                        break;
                    }
                }
        ...
        vi us;
        if(mp[P(vs[0],vs.back())]){
            int is=0;
            repi(j,2,vs.size()-2){
                if(!mp[P(vs[0],vs[j])] && !mp[P(vs[j+1],vs.back())]){
                    is=1;
                    repi(k,j+1,vs.size())us.push_back(vs[k]);
                    int popnum=vs.size()-1-j;
                    rep(k,popnum)vs.pop_back();
                    break;
                }
            }
           // assert(is);
        ...
        vp edges;
        rep(i,vs.size()){
            edges.push_back(P(vs[i],vs[(i+1)%vs.size()]));
        }
        rep(i,us.size()){
            edges.push_back(P(us[i],us[(i+1)%us.size()]));
        }
        while(edges.size()>m){
            edges.pop_back();
        }
        ...
        vp edges;
        vp pa;
        rep(i,n)repi(j,i+1,n)pa.push_back(P(i,j));

        int exs=0;
        auto dfs=[&](auto dfs,int i,int num)->void{
            if(!exs && (i==n*(n-1)/2 && num==m)){
                exs=1;
                for(auto [x,y]:edges){
                    cout<< min(x,y)+1 << " " << max(x,y)+1 << endl;
                }
            ...
            if(i==n*(n-1)/2)return;
            if(exs)return;
            int v=pa[i].first,u=pa[i].second;
            if(!mp[P(v,u)] && num+1<=m && deg[v]+1<=2 && deg[u]+1<=2){
                deg[v]++;
                deg[u]++;
