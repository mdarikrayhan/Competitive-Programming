...
ll binary_monotonic(ll startpoint,ll endpoint,ll arr[],int siz)
{
    ll ans=-1;
    while(endpoint>=startpoint)
    {
        ll midpoint=startpoint+(endpoint-startpoint)/2;
        if(can(midpoint,arr,siz))
        {
            ans=midpoint;
            endpoint=midpoint-1;
        }
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(int c=0; c<n; ++c)
        {
            cin>>arr[c].first.first>>arr[c].first.second>>arr[c].second;
        }
        ll ans=0;
        for(ll c=min(x1,x2); c<=max(x1,x2); ++c)
        {
            if(c==x1||c==x2)
            {
                for(ll v=min(y1,y2); v<=max(y1,y2); ++v)
                {
                    int flag=0;
                    for(int x=0; x<n; ++x)
                    {
                        if(pow(c-arr[x].first.first,2)+pow(v-arr[x].first.second,2)<=
                                pow(arr[x].second,2))
                        {
                            flag=1;
                            break;
            ...
            else
            {
                ...
                for(int x=0; x<n; ++x)
                {
                    if(pow(c-arr[x].first.first,2)+pow(y1-arr[x].first.second,2)<=
                            pow(arr[x].second,2))
                    {
                        flag=1;
                        break;
                ...
                for(int x=0; x<n; ++x)
                {
                    if(pow(c-arr[x].first.first,2)+pow(y2-arr[x].first.second,2)<=
                            pow(arr[x].second,2))
                    {
                        flag=1;
                        break;
