...
#line 1 "/home/maspy/compro/library/ds/offline_query/mo.hpp"
// Nsqrt(Q)
struct Mo {
  vc<pair<int, int>> LR;
...
予想問題概要
・(e_1,...,e_n,1,...,1,2,...,2,...,m,...,m) : 1 から m を k 個ずつ追加
・ここからランダムに r-l+1 個を並べる
・e[l]...e[r] が得られる確率を出力
...
void AC() {
  ...
  FOR(Q) {
    ...
    FOR(i, N) ans *= S - i;
    FOR(i, R - L) ans /= S - i;
    FOR(i, M) ans *= fact<mint>(A[i]);
    FOR(i, M) ans *= fact_inv<mint>(A[i] - B[i]);
    print(ans);
  }
}
...
欲しいもの
prod fact_inv(A[i]+K-B[i])

Mo
集合 {A[i]-B[i]} をうまく管理する,
種類数が少ない.

fact_inv の pow を sqrt 個くらいかける
pow は大丈夫？
*/
...
void solve() {
  ...
  for (auto& K: ks) {
    ...
    mint base = 1;
    FOR(i, N) base *= S - i;
    FOR(i, M) base *= fact<mint>(A[i] + K);
    vc<mint> div(N + 1);
    div[0] = 1;
    FOR(i, N) div[i + 1] = div[i] * (S - i);
    FOR(q, Q) {
      auto [l, r, k] = query[q];
      if (k == K) ANS[q] = base / div[r - l];
    }
  }
  ...
  //   vc<int> B(M);
  //   FOR(i, L, R) B[E[i]]++;
  //   FOR(i, M) ANS[q] *= fact_inv<mint>(A[i] - B[i] + K);
  //   print(ANS[q]);
  // }
  Mo mo;
  FOR(q, Q) {
  ...
  vc<int> A_CNT(N + 1);
  FastSet FS(N + 1);
  for (auto& a: A) A_CNT[a]++, FS.insert(a);
  ...
  auto CALC = [&](int q) -> void {
    auto [L, R, K] = query[q];
    // FOR(i, M) ANS[q] *= fact_inv<mint>(A[i] + K);
    // FOR(a, N + 1) { ANS[q] *= fact_inv<mint>(a + K).pow(A_CNT[a]); }
    FS.enumerate(0, N + 1, [&](int a) -> void {
      if (A_CNT[a] == 0) {
        FS.erase(a);
        return;
      }
      ANS[q] *= fact_inv<mint>(a + K).pow(A_CNT[a]);
    });
  };
