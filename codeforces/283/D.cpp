...
void solve(int test){
    int n; cin >> n;
    vll a(n+1);repi(i,1,n+1)cin >> a[i];
    vll cnt2(n+1);
    vll a2(n+1);
    repi(i,1,n+1){
        ll now=a[i];
    ...
    repi(i,1,n+1){
        ...
        for(int j=i+2; j<=n; j++){
            if(a2[i]%a2[j])continue;
            if(a[j]%2==0){
                int ok=0;
                if(cnt2[j]<=j-i-1)ok=1;
                if(cnt2[j]==cnt2[i]+j-i)ok=1;
                if(ok)chmin(dp[j],now+j-1-i);
            }
            else {
                if(a2[i]%a2[j]==0){
                    chmin(dp[j],now+j-1-i);
                }
            }
        }
    }
    ll ans=n;
    repi(i,1,n+1)chmin(ans,dp[i]+n-i);
    cout << ans << endl;
}
//g++ cf.cpp -std=c++17 -I .
