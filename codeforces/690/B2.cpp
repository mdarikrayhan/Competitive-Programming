...
void solve(ll N) {
  ...
  vc<pi> dat;
  FOR(x, N - 1) FOR(y, N - 1) {
    if (G[x + 0][y + 0] == '0') continue;
    if (G[x + 0][y + 1] == '0') continue;
    if (G[x + 1][y + 0] == '0') continue;
    if (G[x + 1][y + 1] == '0') continue;
    dat.eb(x + 1, y + 1);
  }
  auto I = ConvexHull(dat, "full");
  dat = rearrange(dat, I);

  for (auto& [x, y]: dat) swap(x, y);

  // reverse(all(dat));

  rotate(dat.begin(), min_element(all(dat)), dat.end());
  print(len(dat));
  for (auto& [x, y]: dat) print(x, y);
}
