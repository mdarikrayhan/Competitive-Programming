...
void solve(int test){
    ...
    rep(i,m){
        if(ti[i]==1){
            ...
            if(vs.size()+1<maxsize)vs.push_back(l[i]);
            nowlen++;
        }
        else {
            while(1){
                if(it==n)break;
                if(len[it]<=nowlen+l[i]*r[i]){
                    ll ind=(len[it]-nowlen-1)%l[i]+1;
                    ans[it]=vs[ind];
                    it++;
                }
                else break;
            }
            for(ll j=1; j<=l[i]*r[i]; j++){
                if(vs.size()+1<maxsize){
                    vs.push_back(vs[(j-1)%l[i]+1]);
                }
                else break;
            }
            nowlen+=l[i]*r[i];
