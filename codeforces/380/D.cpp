...
void solve(int test){
    ...
    {
        int all=1;
        rep(i,n)if(a[i])all=0;
        if(all){
            mint ans=0;
            for(int i=1; i<=n; i++){
                ans+=comb(i-1+(n-i),i-1);
            }
            cout << ans.val() << endl;
            return;
        }
    ...
    vp vs;
    rep(i,n+1)if(a[i])vs.push_back(P(a[i],i));
    sort(all(vs));
    vm dp(2);
    int si=vs.size();
    {
        ...
        if(ti!=1 && id+(ti-1)<n){
            ...
            if(si==1)dp[1]=1;
                ...
                for(int i=1; i<=si-1; i++){
                    dp[1]+=comb(i-1+(si-1-i),i-1);
                }
            }
        }
    }
    repi(i,1,si){
        int nowid=vs[i].second;
        int nowti=a[nowid];
        int preid=vs[i-1].second;
        int preti=a[preid];
        int ti=nowti-preti;
        int d=abs(nowid-preid);
        vm old(2);
        rep(j,2)old[j]=dp[j],dp[j]=0;
        if(preid<nowid && nowid-(nowti-1)>=0){
            rep(j,2){
                mint now=old[j];
                ...
                if(j==0){
                    ...
                    if(preid+preti-1<nowid){
                        int x=nowid-(preid+preti-1)-1;
                        int y=ti-1-x;
                        dp[0]+=now*comb(x+y,y);
                    }
                }
            ...
            rep(j,2){
                ...
                if(j==1){
                    ...
                    if(nowid<preid-(preti-1)){
                        int x=preid-(preti-1)-nowid-1;
                        int y=ti-1-x;
                        dp[1]+=now*comb(x+y,y);
                    }
                }
