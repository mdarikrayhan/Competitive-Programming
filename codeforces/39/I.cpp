...
t および集合 S を作れ: 時刻 kt に必ず S 内の点に居る
S を最小化せよ

このとき 0 から到達可能な点全体に mod t のポテンシャルがある.
そんだけか
*/
...
void solve() {
  ...
  int g = 0;
  for (auto& e: G.edges) {
    if (dist[e.frm] == infty<int>) continue;
    int x = dist[e.frm] + 1, y = dist[e.to];
    g = gcd(g, abs(x - y));
  }
  assert(g > 0);
  int t = g;
  vc<int> S;
  FOR(v, N) {
    if (dist[v] < infty<int> && dist[v] % t == 0) S.eb(v);
  }
