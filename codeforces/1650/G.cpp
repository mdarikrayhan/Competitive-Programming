...
auto bfs(int s, const auto & adj) {
    const int n = isz(adj)-1;
    vi dist(1+n, INF);
    dist[s] = 0;
    std::queue<int> q;
    q.push(s);
    ...
    return std::make_pair(std::move(dist),std::move(cnt));
}
...
int main() {
    ...
    while (tt --> 0) {
        ...
        auto [distS,cntS] = bfs(s,adj);
        auto [distT,cntT] = bfs(t,adj);
        const int shortest = distS[t];
        Int answ = cntS[t]; // пути длины ровно shortest
        for (int u = 1; u <= n; u++) {
            for (int v : adj[u]) {
                if (distS[u] == distS[v] && distS[u] + distT[v] == shortest) {
                    // пути shortest + 1 (по ребру u->v)
                    answ += cntS[u] * cntT[v];
                }
            }
        }
