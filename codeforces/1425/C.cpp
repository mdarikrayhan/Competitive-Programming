...
#line 1 "/home/maspy/compro/library/mod/crt3.hpp"
...
#line 3 "/home/maspy/compro/library/poly/coef_of_rational_fps.hpp"
...
#line 1 "/home/maspy/compro/library/other/knight_distance.hpp"
...
// raw[x][y] = A[y-x][2x-y], A[i][j] = raw[i+j][2i+j]
// raw[x][y] = B[x][y-2x], B[i][j] = raw[i][2i+j]

// x,y small ok
// x<=y<2x, 2x<=y で分けて線形漸化式いけると思う

void init() {
  int L = 200;
  vv(int, dist, L, L);
  FOR(x, L) FOR(y, L) dist[x][y] = knight_distance(x, y);
  FOR(x, 1, L) FOR(y, L) dist[x][y] += dist[x - 1][y];
  FOR(x, L) FOR(y, 1, L) dist[x][y] += dist[x][y - 1];
  vv(mint, tmp, 200, 200);
  FOR(x, L - 1) FOR(y, L - 1) tmp[1 + x][1 + y] += dist[x][y];
  FOR(x, 50) FOR(y, 50) raw[x][y] = tmp[x][y];
  FOR(x, 50) FOR(y, 50) A[x][y] = tmp[x + y][2 * x + y];
  FOR(i, 50) FOR(j, 50) B[i][j] = tmp[i][2 * i + j];
}

mint calc(vvc<mint>& A, ll x, ll y) {
  vc<mint> f(50);
  FOR(x0, 50) {
    vc<mint> g(50);
    FOR(i, 50) g[i] = A[x0][i];
    f[x0] = interpolate_linear_rec<mint>(g, y, 10);
  }
  return interpolate_linear_rec<mint>(f, x, 10);
}

mint get(ll x, ll y) {
  if (x > y) swap(x, y);
  mint ANS = 0;
  if (x < 50 && y < 50) return raw[x][y];
  if (y < 2 * x) { return calc(A, y - x, 2 * x - y); }
  return calc(B, x, y - 2 * x);
}
...
void test() {
  FOR(n, 3, 200) {
    FOR(m, 3, 200) {
      mint a = 0, b = get(n, m);
      FOR(x, n) FOR(y, m) a += knight_distance(x, y);
      print(n, m, a, b);
      flush();
      assert(a == b);
    }
