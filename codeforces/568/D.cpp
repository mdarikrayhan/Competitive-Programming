...
vector<Po> P; Line cur, last;
VII res;

int n, mm;

bool safe(Line l){
    ECH(it, P) if (!l.sgn(*it)) return true;
    return false;
}

Line line[N]; set<int> unmatched_line;

bool dfs(int i = 1, int m = mm){

    if (i == n+1){
        //cout << "!!!!" << " " << m << endl;
        ECH(it, unmatched_line) res.PB(MP(*it, -1));
        return true;
    }
    else{

        Line l = line[i]; if (safe(l)) return dfs(i+1, m);

        VI matched; ECH(it, unmatched_line){
            //cout << "!" << *it << " " << l.sgn(line[*it]) << endl;
            if (l.sgn(line[*it])) matched.PB(*it);
        }
        ...
        VII ress = res;
        set<int> _unmatched_line = unmatched_line;
        vector<Po> PP = P;

            ECH(it, matched){
                //cout << " (" << res.size() << " " << m << " " << endl;
                res.PB(MP(*it, i)); P.PB(line[*it]*l);
                unmatched_line.erase(unmatched_line.find(*it));
                if (dfs(i+1, m)) return true;
                    //cout << " (" << res.size() << " " << m << " " << endl;
                P = PP; res = ress; unmatched_line = _unmatched_line;
                //cout << " )" << res.size() << " " << m << " " << endl;
            }
...
int main(){
    ...
    REP_1(i, n){
        LL A, B, C; RDD(A, B, C);
        Line l; if (B) l = Line(Po(0, (DB)-C/B), Po(1, (DB)(-C-A)/B));
        else l = Line(Po((DB)-C/A, 0), Po((DB)(-C-B)/A, 1));
        line[i] = l;

        //cout << l << endl;
    }
    //cout << line[1].sgn(line[2]) << endl;
