...
struct node
{
    int sum = 0, maxp = 0, maxs = 0;
    node() : sum(0), maxp(0), maxs(0) {};
};
struct SegTree
{
    ll N;
    ...
    {
        cur.maxp = max(l.maxp, l.sum + r.maxp);
        cur.maxs = max(r.maxs, r.sum + l.maxs);
        cur.sum = l.sum + r.sum;
    }
    ...
    void pUpdate(ll ver,ll L,ll R,ll pos,ll val)        //set
    {
        if(L==R)
        {
            st[ver].sum = st[ver].maxp = st[ver].maxs = val;
            return;
        }
        ll M=(L+R)/2;
...
int main()
{
    ...
    for(int i = 1; i <= n; i ++)    rq.update(i, 1);
    ...
    for(int x = 1; x <= n; x ++)
    {
        for(auto i : ind[x])
        {
            ...
            if(i != 1)  l = rq.query(1, i - 1).maxs;
            if(i != n)  r = rq.query(i + 1, n).maxp;

            // lets take this as fukin -1
            if((l + r + 1) % 2) ans[i] = max(ans[i], (l + r)/2);
            else    ans[i] = max(ans[i], (l + r + 1)/2);
        }

        for(auto i : ind[x])    rq.update(i, -1);
    }
    ...
    for(int x = n; x >= 1; x --)
    {
        for(auto i : ind[x])
        {
            ...
            if((l + r + 1) % 2) ans[i] = max(ans[i], (l + r)/2);
            else    ans[i] = max(ans[i], (l + r + 1)/2 - 1);
        }
