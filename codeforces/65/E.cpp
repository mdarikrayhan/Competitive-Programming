...
C が大きな成分から使っていくと、最後以外は prefix sum >= 0

実装は？
dfs して、後退辺発生 / 辺終了時に新しい成分に進む
*/
...
void solve() {
  ...
  pq<pair<int, int>> que;
  vc<int> num_e(N);
  for (auto& e: G.edges) num_e[uf[e.frm]]++;
  FOR(v, N) {
    if (uf[v] == v && uf[v] != uf[0]) { que.emplace(num_e[v], v); }
  }

  // 0,u,v move
  // 1,eid,to change edge
  ...
  if (uf.size(0) == 1) {
    ...
    auto [w, eid] = [&]() -> pair<int, int> {
      auto [vs, es] = find_cycle_undirected(G);
      if (vs.empty()) {
        FOR(v, N) {
          if (uf[v] == uf[w0] && G.deg(v) >= 2) {
            for (auto& e: G[v]) return {v, e.id};
          }
        }
        assert(0);
      }
    ...
    FOR(v, 1, N) {
      if (uf[v] == v && uf[v] != uf[r]) { que.emplace(num_e[v], v); }
    }
  }
