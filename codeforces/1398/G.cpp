...
// #pragma GCC optimization("01, 02, 03, 0fast, unroll-loops")
...
// #define int int64_t;         //take note

const int MAXN = 2e5+5, MAXQ = 1e6+5;


int n, y, q;
bitset<MAXN> seg, have;
int ans[MAXQ];
...
int32_t main()
{
    ...
    for(int i = 1, a; i <= n + 1; i ++) cin >> a, seg.set(a);
    
    int cnt = seg.count();
    for(int done = 0; done < cnt; done ++)
    {
        have |= seg;

        int nxt = seg._Find_next(0);
        seg >>= nxt;
    }

    for(int i = 1; i < MAXN; i ++)  if(have[i])
    {
        const int f = i + y, f2 = (i + y) << 1;
        for(int j = f; j < MAXQ; j += f)
            ans[j] = f2;
    }
    cin >> q;
    while(q --)
