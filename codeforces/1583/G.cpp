...
int n, t;
set<int> s;
vector<int> a(MAXN), b(MAXN), id(2*MAXN);
vector<Mint> dp(2*MAXN, 0);
...
int32_t main()
{
    ...
    cin >> n;
    for(int i = 1; i <= n; i ++)    cin >> a[i] >> b[i], id[a[i]] = -i, id[b[i]] = i;
    id[2*n + 1] = n + 1;

    cin >> t;
    for(int i = 1, x; i <= t; i ++) cin >> x, s.insert(x);

    FenwickTreeOneBasedIndexing rq(2*MAXN);

    set<int> behind = {2*n + 1};
    ...
    for(int i = 1; i <= 2*n; i ++)
    {
        ...
        if(x > 0)
        {
            int last = *behind.upper_bound(a[x]);

            dp[b[x]] = 1 + rq.sum(last, 2*n);
            rq.add(a[x], dp[b[x]]);
    ...
    int last = 0;
    for(auto task : s)  last = max(last, b[task]);
    int first = a[id[last]];
    
    Mint ans = 0;
    for(int i = 1; i <= last; i ++) ans += dp[i];

    while(first < last)
    {
        if(id[last] > 0 and a[id[last]] >= first)
        {
            if(s.count(id[last]))
                first = a[id[last]];
            else
